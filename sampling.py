...
class Sampler(SimpleExtension, SamplingBase):
    ...
    def dotests(self,input_sentence_mask):
        
        configuration = configurations.get_config_en2zh()

        if not self.src_vocab:
            self.src_vocab = ensure_special_tokens(cPickle.load(open(configuration['src_vocab'],'rb')), eos_idx=199999)
        if not self.trg_vocab:
            self.trg_vocab = ensure_special_tokens(cPickle.load(open(configuration['trg_vocab'],'rb')), eos_idx=6539)
        ...
        batch = next(input_sentence_mask.get_epoch_iterator(as_dict=True))
        src_batch=batch['source']
        input_= src_batch[0]
        input_length = self._get_true_length(input_, self.src_vocab)
