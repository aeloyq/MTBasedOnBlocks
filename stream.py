...
class _oov_to_unk(object):
    ...
    def __init__(self, src_vocab_size=200000, trg_vocab_size=6540,
                 unk_id=1):
...
def get_tr_stream(src_vocab, trg_vocab, src_data, trg_data,
                  src_vocab_size=200000, trg_vocab_size=6540, unk_id=1,
...
def get_dev_stream(val_set=None, src_vocab=None, src_vocab_size=200000,
                   unk_id=1, **kwargs):
...
def get_test_stream(test_set=None, src_vocab=None, trg_vocab=None, src_vocab_size=200000, trg_vocab_size=6540
    ...
    # Build a batched version of stream to read k batches ahead
    stream = Batch(stream,
                   iteration_scheme=ConstantScheme(sort_k_batches))
    # Convert it into a stream again
    stream = Unpack(stream)
     # Construct batches from the stream with specified batch size
    stream = Batch(
        stream, iteration_scheme=ConstantScheme(1))
    # Pad sequences that are short
    masked_stream = PaddingWithEOS(
        stream, [src_vocab_size - 1, trg_vocab_size - 1])
